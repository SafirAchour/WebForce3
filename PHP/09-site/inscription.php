...
if(!empty($_POST)){
	...
	/* La fonction preg_match vérifie que la chaîne de caractères contenu dans le code postal correspond à l'expression régulière. En cas de succès, elle renvoie 1, sinon elle renvoie 0.
	...
	*/
	...
	// Si pas d'erreur sur le formulaire, on vérifize que le pseudo est unique :
	if(empty($contenu)){
		...
		if ($membre->rowCount() > 0){
			...
			// si la requête ne contient pas ligne, le pseudo est disponible : on l'insère en bdd :
...
if(!$inscription) :
	...
	<form method="post" action="">
		...
		<textarea id="adresse" name="adresse"><?php if(isset($_POST['adresse'])) echo $_POST['adresse']; ?></textarea><br><br>

		<input type="submit" name="inscription" value="s'incrire" class="btn">
